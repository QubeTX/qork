...
fn main() {
    ...
    std::fs::write(man_dir.join("qork.1"), &buffer).expect("Failed to write man page");

    // Also mirror into the project-root man/ directory for release packaging.
    // Best-effort: a read-only source tree (a locked-down cargo registry cache,
    // or a sandboxed packaging step) must still build, so a failure here is
    // non-fatal — the OUT_DIR copy above is authoritative.
    if let Ok(manifest_dir) = std::env::var("CARGO_MANIFEST_DIR") {
        let project_man = std::path::PathBuf::from(manifest_dir).join("man");
        std::fs::create_dir_all(&project_man).ok();
        std::fs::write(project_man.join("qork.1"), buffer).ok();
    }
}
